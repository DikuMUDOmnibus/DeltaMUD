   ...
   header (i.e. the text handed to the player) */
{
  ...
  *(tmstr + strlen (tmstr) - 1) = '\0';
  ...
  sprintf (buf, " &b-&c=&y Deltanian Postal Service&c =&b-&n\r\n"
	   "&GD&gate&c:&n %s\r\n"
	   "  &GT&go&c:&n %s\r\n"
		      ...
		      int cmd, char *arg)
{
  ...
  if (GET_LEVEL (ch) < MIN_MAIL_LEVEL && GET_LEVEL(mailman) != 1)
    {
  ...
  if (!*buf)
    {				/* you'll get no argument from me! */
      act ("$n tells you, 'You need to specify an address!'",
	   FALSE, mailman, 0, ch, TO_VICT);
      return;
    }
  if (GET_GOLD (ch) < STAMP_PRICE && GET_LEVEL(ch) < LVL_IMMORT && GET_LEVEL(mailman) != 1)
    {
