...
ACMD(do_brew)
{
	...
	/* Can't make scrolls in arena */
	if (IS_ARENACOMBATANT(ch)){
	  send_to_char("Strangely enough, you find your ability to brew potions dimunished while you're in the arena.\r\n",ch);
	  return;
	}

	if (!(GET_CLASS(ch) == CLASS_ARTISAN || GET_LEVEL(ch) >= LVL_IMMORT)) {
...
void make_scroll(struct char_data *ch, int scroll, struct obj_data *paper)
{
	...
	sprintf(buf2, "A parchment inscribed with the runes '%s' lies here.", garble_spell(scroll));
...
ACMD(do_scribe)
{
	...
	/* Can't make scrolls in arena */
	if (IS_ARENACOMBATANT(ch)){
	  send_to_char("Strangely enough, you find your ability to scribe scrolls diminished while you're in the arena.\r\n",ch);
	  return;
	}
	...
	if (!(GET_CLASS(ch) == CLASS_ARTISAN || GET_LEVEL(ch) >= LVL_IMMORT)) {
        ...
        if (GET_SKILL(ch, SKILL_SCRIBE) == 0) {
        send_to_char("You have no idea how to scribe scrolls!\r\n", ch);
             return;
}
...
ACMD(do_forge)
{
	...
	*/
	...
	if ((GET_OBJ_VAL(weapon, 0) > 0) && (GET_LEVEL(ch) < LVL_GOD)) {
	...
	prob += ((GET_DEX(ch) - 11) << 1);
	prob += ((GET_STR(ch) - 11) << 1) + (GET_ADD(ch) >> 3);
	prob -= number(10,30);

	if ((number((prob*0.7), (prob*1.5)) > 
	     (prob+(GET_LEVEL(ch)/number(5,10)))) 
	...
	if (dice (GET_OBJ_VAL (weapon, 1), GET_OBJ_VAL (weapon, 2)) > 
	    (GET_LEVEL(ch)/2.5)){
	  send_to_char("As you pound out the dents in the weapon,"
		       " you stress it beyond it's ability and it explodes!\r\n", ch);
	  act("$n tries to forge a weapon, but it explodes!",
	      FALSE, ch, 0,0, TO_ROOM);
	  extract_obj(weapon);
	  dam = number(10, 50);
	...
	GET_OBJ_RENT(weapon) *= number(2,5);
	/* GET_OBJ_RENT(weapon) += GET_LEVEL(ch) * 50 * number(5,10); */
