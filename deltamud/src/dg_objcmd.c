...
long find_obj_target_room(obj_data *obj, char *rawroomstr)
{
    int tmp;
    long location;
    ...
    if (!*roomstr)
	return NOWHERE;
    if ((location=cdsr(roomstr))!=-1) return location;
...
obj_data *cycle_up (struct obj_data *obj) {
  if (obj->in_obj)
    return cycle_up(obj->in_obj);
  else
    return obj;
}
// Cycle up -- get the parent obj container of the curr obj.
OCMD(do_odamage) {
    char name[MAX_INPUT_LENGTH], amount[MAX_INPUT_LENGTH];
    int dam = 0, all=0;
    ...
    if (!*name || !*amount || (!isdigit(*amount) && (*amount=='-' && !isdigit(*(amount+1))))) {
    ...
    if (!strcmp(name, "all")) {
      ...
      tobj=cycle_up(obj);
      if (tobj->worn_by)
        in_room=tobj->worn_by->in_room;
      if (tobj->carried_by)
        in_room=tobj->carried_by->in_room;
      if (in_room==-1) return;
    }

    for (ch = all ? world[in_room].people : get_char_by_obj(obj, name); all ? (int) ch : 1;
ch=nextch) {
        if (!ch) {
	...
	if (GET_POS(ch) == POS_DEAD) {
	    if (!IS_NPC(ch)) {
		sprintf(buf2, "%s killed by %s [%s] at %s", GET_NAME(ch),
	      		obj->short_description, obj->carried_by ? GET_NAME(obj->carried_by) :
                        (obj->worn_by ? GET_NAME(obj->worn_by) : "No Owner"), world[ch->in_room].name);
      		mudlog(buf2, BRF, 0, TRUE);
	    }
    	    die(ch, ch != (obj->carried_by ? obj->carried_by : obj->worn_by) ? (obj->carried_by ?
obj->carried_by : obj->worn_by) : NULL);
	}
        if (!all) return;
    }
}
