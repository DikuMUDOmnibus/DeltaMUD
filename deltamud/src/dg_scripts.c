...
/* external functions */
long find_target_room(char_data * ch, char *rawroomstr);
void free_varlist(struct trig_var_data *vd);
int obj_room(obj_data *obj);
int is_empty(int zone_nr);
long find_target_room(struct char_data * ch, char *rawroomstr);
...
void script_trigger_check(void)
{
  ...
  for (ch = character_list; ch; ch = ch->next) {
    if ((sc=SCRIPT(ch)))
      if (IS_SET(SCRIPT_TYPES(sc), WTRIG_RANDOM) && (!is_empty(world[IN_ROOM(ch)].zone) || IS_SET(SCRIPT_TYPES(sc), WTRIG_GLOBAL)))
	random_mtrigger(ch);
  }
  ...
  if (is_abbrev(arg, "mtr")) {
	...
	if ((rn >= 0) && (trig = read_trigger(rn))) {
	  ...
	  sprintf(buf, "Trigger %d (%s) attached to room %d.\r\n",
		  tn, GET_TRIG_NAME(trig), (int) world[room].number);
	  send_to_char(buf, ch);
	} else
	  send_to_char("That trigger does not exist.\r\n", ch);
      }
...
/* deletes a variable with given name and value to trigger */
void del_var(struct trig_var_data **var_list, char *name)
{
  struct trig_var_data *vd, *lastvd=0;

  for (vd = *var_list; vd && str_cmp(vd->next->name, name); vd = vd->next) lastvd=vd;
                ...
                int type, char *var, char *field, char *subfield, char *str)
{
  ...
  if (!*field) {
	...
	*str = '\0';
	...
	if (!str_cmp(field, "char")) {
	  ...
	  if (rndm)
	    ...
	    *str = '\0';
        ...
        else if (!str_cmp(field, "letter"))
          sprintf(str, "%c", 96+number(1,26));
	else
	...
	sprintf(str, "%d", (int) world[IN_ROOM(c)].number);
	...
	sprintf(str, "%s", o->short_description);
