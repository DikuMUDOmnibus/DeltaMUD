...
{CNRM, CRED, CGRN, CYEL, CBLU, CMAG, CCYN, CWHT,
...
void
proc_color (char *inbuf, int colour)
{
  register int j = 0, p = 0;
  int k, max, c = 0, lastcol=0, lastcol2=0;
  char out_buf[32768];
  ...
  while (inbuf[j] != '\0')
	  ...
	  && isnum (inbuf[j + 2]) && isnum (inbuf[j + 3]))
	{
      ...
      else if ((inbuf[j] == '&') && (!(is_colour (inbuf[j + 1]) == -1) || LOWER(inbuf[j+1])=='l'))
	{
          if (LOWER(inbuf[j+1])=='l')
            c = colour != -1 ? lastcol2 : number(1,14);
          else
	    c = colour != -1 ? is_colour (inbuf[j + 1]) : number(1, 14);
          lastcol2=lastcol;
          lastcol=c;
	  j += 2;
	}
