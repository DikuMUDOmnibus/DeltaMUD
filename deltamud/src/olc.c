...
struct olc_scmd_data olc_scmd_info[8] =
{
  ...
  */
  ...
  ACMD(do_olc)
  {
   ...
   if (subcmd == SCMD_OLC_REDIT && MAP_ACTIVE && world[map_start_room].number == number) {
     send_to_char("NO EDITING THE MAP GODDAMNIT!\r\n", ch);
     return;
   }
   ...
   if (save) {
     ...
     case SCMD_OLC_TRIGEDIT:
       send_to_char("Triggers are autosaved to disk when edited, there's no need.\r\n", ch);
       return;
     }
     if (!type) {
...
void olc_saveinfo(struct char_data *ch)
{
   ...
   buf2[0]='\0';
   if (olc_save_list)
     sprintf(buf2, "The following OLC components need saving:\r\n");
   else {
     send_to_char("The database is up to date.\r\n", ch);
     return;
   }
   for (entry = olc_save_list; entry; entry = entry->next) {
     if ((int)entry->type == OLC_SAVE_HELP && GET_LEVEL(ch)>=LVL_IMMORT)
       sprintf(buf, " - Help Entries.\r\n");
     else if ((int)entry->type == OLC_SAVE_ACTION && GET_LEVEL(ch)>=LVL_IMMORT)
       sprintf(buf, " - Actions.\r\n");
     else if (can_edit_zone(ch, real_zone(entry->zone*100)) || GET_LEVEL(ch)>=LVL_IMMORT)
...
/*  if (GET_LEVEL(ch) < LVL_BUILDER)
    return FALSE; We're gonna have mort. builders right? */

  if (number < 0 || number > top_of_zone_table) return FALSE;
...
#define CLASS_APPMODNUM(num, obj) ( 750 * class_strengths[obj.obj_class+1][num] * obj.min_level / 5 / 100 / NUM_WEARS)

ACMD(do_rebalance) {
  int zone, endzone, tobalance, i;
  extern int top_of_objt;
  extern int top_of_mobt;
  extern struct obj_data *obj_proto;
  extern int class_strengths[6][5];
  extern void set_mob_stats (struct char_data *ch, byte level);

  buf[0]=buf2[0]=0;

  two_arguments(argument, buf, buf2);
  zone=real_zone(atoi(buf2)*100);

  if (!*buf || !*buf2) {
    send_to_char("Format: rebalance <mob/obj> <znum>\r\n", ch);
    return;
  }

  if (is_abbrev(buf, "obj")) tobalance=1;
  else if (is_abbrev(buf, "mob")) tobalance=2;
  else {
    send_to_char("Balance mobs or objects?\r\n", ch);
    return;
  }

  if (!isdigit(buf2[0]) || zone == -1) {
    send_to_char("Invalid zone number.\r\n", ch);
    return;
  }

  if (!can_edit_zone(ch, zone)) {
    send_to_char("You do not have permission to edit that zone.\r\n", ch);
    return;
  }

  sprintf(buf, "%s completely rebalanced %s in zone %d.", GET_NAME(ch), tobalance == 1 ? "objects" : "mobiles", zone_table[zone].number);
  mudlog(buf, NRM, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE);
  sprintf(buf, "Rebalancing %s in zone %d: You will have to save this zone for changes to be permanent.", tobalance == 1 ? "objects" : "mobiles", zone_table[zone].number);
  send_to_char(buf, ch);

  olc_add_to_save_list(zone_table[zone].number, tobalance == 1 ? OLC_SAVE_OBJ : OLC_SAVE_MOB);

  endzone=zone_table[zone].top; /* End vnum of zone. */
  zone=zone_table[zone].number*100; /* Start vnum of zone. */

  switch(tobalance) {
    case 1:
      for (i=0; i<=top_of_objt; i++)
        if (obj_index[obj_proto[i].item_number].vnum>=zone) {
          if (obj_index[obj_proto[i].item_number].vnum>endzone)
            break;
          if (!obj_proto[i].min_level) continue;
          obj_proto[i].affected[0].location=APPLY_POWER;
          obj_proto[i].affected[0].modifier=CLASS_APPMODNUM(0, obj_proto[i]);
          obj_proto[i].affected[1].location=APPLY_MPOWER;
          obj_proto[i].affected[1].modifier=CLASS_APPMODNUM(1, obj_proto[i]);
          obj_proto[i].affected[2].location=APPLY_DEFENSE;
          obj_proto[i].affected[2].modifier=CLASS_APPMODNUM(2, obj_proto[i]);
          obj_proto[i].affected[3].location=APPLY_MDEFENSE;
          obj_proto[i].affected[3].modifier=CLASS_APPMODNUM(3, obj_proto[i]);
          obj_proto[i].affected[4].location=APPLY_TECHNIQUE;
          obj_proto[i].affected[4].modifier=CLASS_APPMODNUM(4, obj_proto[i]);
        }
      break;

    case 2:
      for (i=0; i<=top_of_mobt; i++)
        if (mob_index[mob_proto[i].nr].vnum>=zone) {
          if (mob_index[mob_proto[i].nr].vnum>endzone)
            break;
          set_mob_stats(&mob_proto[i], GET_LEVEL(&mob_proto[i]));
        }
      break;
