...
void oedit_disp_prompt_apply_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf, " %s%d%s) Autoset object applies.", grn, counter + 1, nrm);
  send_to_char(buf, d->character);
...
/*  case ITEM_WEAPON:
   ...
   */
  case ITEM_ARMOR:
    send_to_char("Apply to Defense : ", d->character);
    break;
  case ITEM_CONTAINER:
...
void oedit_disp_perm_affects_menu(struct descriptor_data *d)
{
  int counter, columns = 0;
...
void oedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	  ...
	  "%sE%s) Extra descriptions menu\r\n"
          ...
          "%sG%s) Maximum Durability : %s%d\r\n"
          "%sH%s) Permanent Affects : %s%s\r\n"
          "%sK%s) Intended Class : %s%s\r\n"
          "%sL%s) Level       : %s%d\r\n"
          "%sS%s) Script      : %s%s\r\n"
	  "%sQ%s) Quit\r\n"
	  "Enter choice : ",
	  ...
	  grn, nrm, grn, nrm, 
          ...
          grn, nrm, cyn, buf2,
          grn, nrm, cyn, (obj->obj_class == CLASS_UNDEFINED ? "Generic" : (char *) pc_class_types[obj->obj_class]),
          grn, nrm, cyn, obj->min_level,
...
int class_str2num(char *string) {
  int i;
  for (i=0; i<NUM_CLASSES; i++)
    if (is_abbrev(string, pc_class_types[i]))
      return i;
  return -1;
}

int class_strengths[6][5] = {
/* Power | Magic Power | Defense | Magic Defense | Technique -> Strengths on a 1 (worse) to 5 (best) scale. */
  {3,      3,            3,        3,              3        }, /* Generic  */
  {1,      5,            1,        5,              4        }, /* Mage     */
  {2,      3,            4,        4,              3        }, /* Cleric   */
  {4,      1,            4,        2,              5        }, /* Thief    */
...
// #define CLASS_APPMODNUM(num) ( 750 * class_strengths[OLC_OBJ(d)->obj_class+1][num] / 5 * OLC_OBJ(d)->min_level / 100 / NUM_WEARS) <- Original, we placed all the divides at the end because they cause calculation innaccuracy when dealing with integers.
#define CLASS_APPMODNUM(num) ( 750 * class_strengths[OLC_OBJ(d)->obj_class+1][num] * OLC_OBJ(d)->min_level / 5 / 100 / NUM_WEARS)

void autoset_obj_applies (struct descriptor_data *d) {
  if (!OLC_OBJ(d)->min_level) {
    send_to_char("Object's minimum level must be set before you can do this.\r\n", d->character);
    return;
  }
  sprintf(buf, "Setting object's standard applies based off of a level %d %s.\r\n", OLC_OBJ(d)->min_level, OLC_OBJ(d)->obj_class == CLASS_UNDEFINED ? "Generic Character" : (char *) pc_class_types[OLC_OBJ(d)->obj_class]);
  send_to_char(buf, d->character);
  OLC_OBJ(d)->affected[0].location=APPLY_POWER;
  OLC_OBJ(d)->affected[0].modifier=CLASS_APPMODNUM(0);
  OLC_OBJ(d)->affected[1].location=APPLY_MPOWER;
  OLC_OBJ(d)->affected[1].modifier=CLASS_APPMODNUM(1);
  OLC_OBJ(d)->affected[2].location=APPLY_DEFENSE;
  OLC_OBJ(d)->affected[2].modifier=CLASS_APPMODNUM(2);
  OLC_OBJ(d)->affected[3].location=APPLY_MDEFENSE;
  OLC_OBJ(d)->affected[3].modifier=CLASS_APPMODNUM(3);
  OLC_OBJ(d)->affected[4].location=APPLY_TECHNIQUE;
  OLC_OBJ(d)->affected[4].modifier=CLASS_APPMODNUM(4);
}
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case OEDIT_MAIN_MENU:
    ...
    case 'f':
    case 'F':
     send_to_char ("Enter numerator : ", d->character);
     OLC_MODE (d) = OEDIT_DURABILITY;
     break;
    case 'g':
    case 'G':
     send_to_char ("Enter denominator : ", d->character);
     OLC_MODE (d) = OEDIT_DURABILITY2;
     break;
    case 'h':
    case 'H':
      if (GET_LEVEL(d->character) < LVL_GRGOD) {
        send_to_char("Your level isn't high enough to do that...\r\n", d->character);
        return;
      }
      oedit_disp_perm_affects_menu(d);
      OLC_MODE(d) = OEDIT_PERM_AFFECTS;
      break;
    case 'k':
    case 'K':
      OLC_MODE(d) = OEDIT_CLASS;
      send_to_char ("Enter Intended Class For Object : ", d->character);
      break;
    case 'l':
    case 'L':
      OLC_MODE(d) = OEDIT_LEVEL;
      send_to_char ("Enter Minimum Level For Object : ", d->character);
      break;
    case 's':
    case 'S':
  ...
  case OEDIT_DURABILITY:
   ...
   case OEDIT_DURABILITY2:
    GET_OBJ_TSLOTS (OLC_OBJ (d)) = atoi (arg);
    break;

  case OEDIT_LEVEL:
    OLC_OBJ(d)->min_level=atoi(arg);
    break;

  case OEDIT_PERM_AFFECTS:
    number = atoi(arg);
    if ((number < 1) || (number > NUM_AFFECT_FLAGS)) {
      if (number==0) break;
      send_to_char("Invalid choice, try again : ", d->character);
      return;
    } else
      TOGGLE_BIT(OLC_OBJ(d)->obj_flags.bitvector, (1 << (number-1)));
    oedit_disp_perm_affects_menu(d);
    return; 

  case OEDIT_CLASS:
    OLC_OBJ(d)->obj_class=class_str2num(arg);
    break;
  ...
  case OEDIT_PROMPT_APPLY:
    if ((number = atoi(arg)) == 0)
      break;
    else if (number < 0 || number > MAX_OBJ_AFFECT+1) {
      oedit_disp_prompt_apply_menu(d);
      return;
    }
    if (number==MAX_OBJ_AFFECT+1) {
      autoset_obj_applies(d);
      oedit_disp_prompt_apply_menu(d);
      return;
    }
    OLC_VAL(d) = number - 1;
