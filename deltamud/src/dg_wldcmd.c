...
void wld_log(room_data *room, char *msg)
{
    ...
    sprintf(buf, "Wld (room %d): %s", (int) room->number, msg);
    script_log(buf);
}
...
WCMD(do_wdoor)
{
	...
	case 1:  /* description */
	    ...
	    break;
	case 2:  /* flags       */
	    exit->exit_info = (long)asciiflag_conv(value);
	    break;
	case 3:  /* key         */
	    exit->key = atoi(value);
	    break;
	case 4:  /* name        */
	    ...
	    break;
	case 5:  /* room        */
	    if ((to_room = real_room(atoi(value))) != NOWHERE || atoi(value) == NOWHERE)
...
WCMD(do_wteleport)
{
    ...
    if ((target=cdsr(arg2))!=-1);
    else {
      if ((ch=get_char_vis(NULL, arg2)))
        target=ch->in_room;
      else if ((tobj=get_obj_vis(NULL, arg2)))
        target=tobj->in_room;
      else
...
WCMD(do_wdamage) {
    ...
    if (!strcmp(name, "all")) all=1;

    for (ch = all ? room->people : get_char_by_room(room, name); all ? (int) ch : 1; ch=nextch) {
        if (!ch) {
