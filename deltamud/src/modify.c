...
void string_add(struct descriptor_data *d, char *str)
{
  ...
  if (terminator) {
    ...
    extern void redit_disp_special_exit_menu(struct descriptor_data *d);
	...
	SEND_TO_Q("Description aborted.\r\n", d);
      ...
      if (d->note) {
        if (terminator == 1) {
          ...
          if (d->backstr)
            free(d->backstr);
          SEND_TO_Q("Note saved.\r\n", d);
        }
        else {
          SEND_TO_Q("Note aborted.\r\n", d);
          if (d->str)
            free(d->str);
          d->note->action_description=d->backstr;
        }
        d->note=NULL;
        act("$n stops writing a note.", TRUE, d->character, 0, 0, TO_ROOM);
        STATE(d)=CON_PLAYING;
        return;
      }
      if (terminator == 1) {
