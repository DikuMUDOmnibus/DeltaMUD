...
void zedit_setup(struct descriptor_data *d, int room_num)
{
  ...
  zone->lvl1 = zone_table[OLC_ZNUM(d)].lvl1;
  zone->lvl2 = zone_table[OLC_ZNUM(d)].lvl2;
  zone->status_mode = zone_table[OLC_ZNUM(d)].status_mode;
  /*
...
void zedit_new_zone(struct char_data *ch, int vzone_num)
{
  ...
  if (vzone_num < 0) {
    ...
    return;
  } else if (vzone_num > MAX_ZONE_NUM) {
    send_to_char("That is higher then highest zone allowed.\r\n", ch);
    return;
  }
...
void zedit_save_internally(struct descriptor_data *d)
{
  ...
  if (OLC_ZONE(d)->number) {
    ...
    zone_table[OLC_ZNUM(d)].lvl1       = OLC_ZONE(d)->lvl1;
    zone_table[OLC_ZNUM(d)].lvl2       = OLC_ZONE(d)->lvl2;
    zone_table[OLC_ZNUM(d)].status_mode = OLC_ZONE(d)->status_mode;
  }
...
void zedit_save_to_disk(int zone_num)
{
  ...
  fprintf(zfile, "#%d\n" "%s~\n" "%s~\n" "%d %d %d\n" "%d %d %d\n",
	  ...
	  zone_table[zone_num].reset_mode,
          zone_table[zone_num].lvl1,
          zone_table[zone_num].lvl2,
          zone_table[zone_num].status_mode
	  );
...
void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	  ...
	  "%sR%s) Reset Mode  : %s%s%s\r\n"
          "%sV%s) Levels      : %s%d%s to %s%d%s\r\n"
          "%sA%s) Approve zone: %s%s%s\r\n"
	  "[Command list]\r\n",
	  ...
	  ((OLC_ZONE(d)->reset_mode == 1) ?
	   ...
	   "Normal reset.") :
 	   "Never reset", nrm,
         grn, nrm, yel, OLC_ZONE(d)->lvl1, nrm, yel, OLC_ZONE(d)->lvl2, nrm,
         grn, nrm, yel, OLC_ZONE(d)->status_mode ?
            "Approved for mortals." : "Closed to mortals.", nrm
	  );
...
void zedit_disp_comtype(struct descriptor_data *d)
{
 ...
 /*-------------------------------------------------------------------*/
 /*. Print the levels menu and setup response catch. */

void zedit_disp_levels(struct descriptor_data *d)
{ get_char_cols(d->character);
  sprintf(buf, ""
    "Set Level 1 at 91 for an immortal zone, set both at 0 for an all level zone.\r\n"
    "%s1%s) Minimum level area designed for: %s%d%s\r\n"
    "%s2%s) Maximum level area designed for: %s%d%s\r\n"
    "Set which level? : ",
    grn, nrm, grn, OLC_ZONE(d)->lvl1, nrm,
    grn, nrm, grn, OLC_ZONE(d)->lvl2, nrm 
  );
  send_to_char(buf, d->character);
  OLC_MODE(d)=ZEDIT_LVLS;
}
  ...
  case ZEDIT_MAIN_MENU:
	...
	OLC_MODE(d) = ZEDIT_ZONE_TOP;
      ...
      break;
       case 'a':
       case 'A':
        /*. Approve zone for mort behaviour ;0 .*/
         if(GET_LEVEL(d->character) < LVL_REVIEW)
           zedit_disp_menu(d);
         else
         { send_to_char("Approve this zone? ", d->character);
           OLC_MODE(d) = ZEDIT_APPROVE;
         }
         break;
       case 'v':
       case 'V':
...
< LVL_GRGOD) {
       send_to_char("You don't have permissions to that zone, try again : ", d->character);
       break; 
    } 
    switch (OLC_CMD(d).command) {
    case 'M':
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MIN_LVL:
    pos = atoi(arg);
    if (!isdigit(*arg)) {
      send_to_char("Value must be an integer value. Minimum level? ", d->character);
     } else {
      if (pos > 100) {
        send_to_char("Value must be below 100. Minimum level? ", d->character);
      } else {
        if (pos < 0) {
          send_to_char("Value must be above 0. Minimum level? ", d->character);
        } else {
          OLC_ZONE(d)->number = 1;
          OLC_ZONE(d)->lvl1 = pos;
        }
...
/*-------------------------------------------------------------------*/
   case ZEDIT_MAX_LVL:
     pos = atoi(arg);
     if (!isdigit(*arg)) {
       send_to_char("Value must be an integer value. Maximum level? ", d->character);
     } else {
       if (pos > 100) {
         send_to_char("Value must be below 100. Maximum level? ", d->character);
       } else {
         if (pos < 0) {
           send_to_char("Value must be above 0. Maximum level? ", d->character);
         } else {
           OLC_ZONE(d)->number = 1;
           OLC_ZONE(d)->lvl2 = pos;
         }
...
/*-------------------------------------------------------------------*/
   case ZEDIT_LVLS:
     /* define what max&min levels this area is designed for */
     pos = atoi(arg);  
     if (isdigit(*arg))
       switch (pos) {
       case 1:
         send_to_char("Minimum level? ", d->character);
         OLC_MODE(d) = ZEDIT_MIN_LVL;
         break;
       case 2:
         send_to_char("Maximum level? ", d->character);
         OLC_MODE(d) = ZEDIT_MAX_LVL;
         break;
       default:
         zedit_disp_levels(d);
       }
...
//      OLC_ZONE(d)->top = MAX(OLC_ZNUM(d) * 100, MIN(32000, atoi(arg)));
     OLC_ZONE(d)->top = MAX(OLC_ZNUM(d) * 100, atoi(arg));
    else
 ...
 /*--------------------------------------------------------------------*/
    case ZEDIT_APPROVE:
      /*. Approve/disapprove area for mortal exploration/exploitation. */
     switch (*arg) {
       case 'y':
       case 'Y':
         OLC_ZONE(d)->status_mode = 1;
         break;
       case 'n':
       case 'N':
         OLC_ZONE(d)->status_mode = 0;
         break;
       default:
         send_to_char("Try again : ", d->character);
         return;
