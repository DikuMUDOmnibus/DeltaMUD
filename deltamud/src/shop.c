...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
      ...
      ((IS_ARTISAN(ch) && NOTRADE_ARTISAN(shop_nr)) ||
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  if ((!pk_allowed) && (PLR_FLAGGED (ch, PLR_KILLER))
    ...
    do_say(keeper, MSG_NO_SELL_PAWN, cmd_say, 0);
    return;
  }
...
int sell_price(struct char_data * ch, struct obj_data * obj, int shop_nr)
{
  ...
  value = (int) (GET_OBJ_COST(obj) * SHOP_SELLPROFIT(shop_nr));

  if ((!pk_allowed) && (PLR_FLAGGED (ch, PLR_KILLER))
      && (ch->in_room == real_room(jail_num)) && (value > 75000)){
    value = 75000;
  }
  return (value);
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  while ((obj) && (GET_GOLD(keeper) + SHOP_BANK(shop_nr) >=
    ...
    if (obj && obj->short_description)
      strcpy(tempstr, obj->short_description);
    obj_from_char(obj);
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  sprintf(buf, "%s I'll give you %s gold coins for that!", GET_NAME(ch),
	  numdisplay(sell_price(ch, obj, shop_nr)));
  do_tell(keeper, buf, cmd_tell, 0);
...
char *list_object(struct obj_data * obj, int cnt, int index, int shop_nr)
{
  ...
  if (shop_producing(obj, shop_nr))
    ...
    sprintf(buf2, "%8s    ", numdisplay(cnt));
  ...
  sprintf(buf2, "%-48s %8s\n\r", buf3, numdisplay(buy_price(obj, shop_nr)));
  strcat(buf, CAP(buf2));
  return (buf);
}
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  for (index = 0; SHOP_ROOM(shop_nr, index) != NOWHERE; index++) {
    ...
    if ((temp = real_room(SHOP_ROOM(shop_nr, index))) != NOWHERE)
      sprintf(buf1, "%s (#%d)", world[temp].name, (int) world[temp].number);
    else
